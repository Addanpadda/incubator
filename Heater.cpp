...
Heater::Heater(const double &temperature, const double &setPoint) {
  ...
  _tempPID->setBangBang(2, 1); // Completely off 1 degree over setpoint, and on 2 degrees under
  //set PID update interval to 4000ms
  //_tempPID->setTimeStep(5000);
}

void Heater::run() const {
  int heatingTime = _cycleTimeMS * _powerFactor;
  int cooldownTime = _cycleTimeMS - heatingTime;
